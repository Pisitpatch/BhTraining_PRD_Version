<%@ Page Language="VB" AutoEventWireup="false" CodeFile="evaluate_page.aspx.vb" Inherits="game_evaluate_page" %>
...
.answer ol
{margin: 0px;  color: #339; font-size: 28px; font-family: Tahoma; font-weight: normal; line-height: 1.85em; padding-left: 150px;}
...
<div>
		  <table width="100%">
		    <tr>
		      ...
		      <td width="285" align="right" class="topbar">Time left <span id = "mytime"></span>&nbsp;sec&nbsp;&nbsp;&nbsp;</td>
	        </tr>
  </table>
	    </div><!-- /header -->

	<div class="content">
    <div class="question" style="margin-top:25px"><span></span> <%Response.Write(question_text) %> </div>
    <div class="answer" style="font-size:16px;margin-top:30px; padding-left:30px">
  	<ol>
     <%
        for i as integer = 0 to dsQuestion.tables(0).rows.count - 1
        %>
       
    <li> <a href="evaluate_page.aspx?order=<%response.write(order) %>&q_order=<%response.write(q_order) %>&gid=<%response.write(gid) %>&lang=<%response.write(lang) %>" rel="external" id="choice<% response.write(i+1) %>"><%Response.Write(dsQuestion.Tables(0).Rows(i)("answer_detail_" & lang).ToString)%></a></li>
