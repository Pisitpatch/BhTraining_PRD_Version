Imports System.Data
Partial Class game_multiple_choice_login
        ...
        If lblError.Text = "" Then
            ...
            If txtgroup.Items.Count > 0 Then
                ...
                If txtusername.Text = "0" Then
                    ...
                    conn.executeSQL(sql)
                End If
                Response.Redirect("multiple-choice-page.aspx?gid=" & txtgroup.SelectedValue & "&lang=" & txtlang.SelectedValue)
            Else
    ...
    Sub setRandomQuestion()
        ...
        Try
            ...
            sql &= " (select top 70 percent question_id from jci_master_question where isnull(is_delete,0) = 0 and isnull(is_active,0) = 1 and group_id = " & txtgroup.SelectedValue & "  order by newid())"
    ...
    Sub bindGroup()
        ...
        Try
            sql = "SELECT * FROM jci_master_group a INNER JOIN jci_master_test b ON a.test_id = b.test_id AND b.is_game = 1 AND ISNULL(a.is_delete,0) = 0 AND category_id = 102 "
            ...
            If Not IsNothing(Session("jci_emp_code")) Then
                ...
                sql &= " AND  x1.game_group_id IN (SELECT group_id FROM jci_ip_target WHERE ip_address = '" & HttpContext.Current.Request.UserHostAddress & "')"
                ' Response.Write(sql)
                'Response.End()
                ds = conn.getDataSet(sql, "t1")
                If ds.Tables("t1").Rows.Count > 0 Then
                ...
                Else
                    loginMsg = "You don't have question set in this zone"
                End If
