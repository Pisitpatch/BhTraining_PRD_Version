...
Partial Class incident_report_viewer
    ...
    Sub bindDefendant()
        ...
        Try
            sql = "SELECT a.dept_unit_name , ISNULL(COUNT(a.dept_unit_name),0) AS num FROM ir_cfb_unit_defendant a INNER JOIN ir_trans_list b ON a.ir_id = b.ir_id AND ISNULL(a.comment_id,0) = 0 "
            sql &= " WHERE MONTH(date_submit) =" & start_month(0) & " AND YEAR(date_submit) = " & start_month(1) 
            sql &= " AND b.status_id > 1 AND ISNULL(b.is_delete,0) = 0 AND ISNULL(b.is_cancel,0) = 0 AND a.dept_unit_id <> 143 "
    ...
    Sub barGraph()
        ...
        Try
            ...
            StrWer.WriteLine("<paragraph align=""center"" bold=""true"" color=""0x000066"" size=""16"" x=""45"" y=""10"" width=""500"" text=""Incident Report by Risk Level"" />")
            ...
            StrWer.WriteLine("<serie borderColor=""0xbd7400"" lightColor=""0xffde00"" darkColor=""0xbd7400"" tooltipBackColor=""0xe6b600"" tooltipTextColor=""0x000000"" name=""Insignificant"" />")
            StrWer.WriteLine("<serie borderColor=""0x425802"" lightColor=""0xc1d681"" darkColor=""0x425802"" tooltipBackColor=""0x93a853"" tooltipTextColor=""0xFFFFFF"" name=""Near Miss"" />")
            StrWer.WriteLine("<serie borderColor=""0x076984"" lightColor=""0x40cff6"" darkColor=""0x06a2cc"" tooltipBackColor=""0xe0323c"" tooltipTextColor=""0xFFFFFF"" name=""No Harm"" />")
            StrWer.WriteLine("<serie borderColor=""0x5e2c8d"" lightColor=""0x9872bb"" darkColor=""0x562e7b""  tooltipBackColor=""0xe0323c"" tooltipTextColor=""0xFFFFFF"" name=""Mild AE"" />")
            StrWer.WriteLine("<serie borderColor=""0xb3036b"" lightColor=""0xf758b6"" darkColor=""0xd60380"" tooltipBackColor=""0xe0323c"" tooltipTextColor=""0xFFFFFF"" name=""Moderate AE"" />")
            StrWer.WriteLine("<serie borderColor=""0xa03f11"" lightColor=""0xe87037"" darkColor=""0xdc4e0a"" tooltipBackColor=""0xe0323c"" tooltipTextColor=""0xFFFFFF"" name=""Serious AE"" />")
            StrWer.WriteLine("<serie borderColor=""0x700408"" lightColor=""0xf03a45"" darkColor=""0x700408"" tooltipBackColor=""0xe0323c"" tooltipTextColor=""0xFFFFFF"" name=""Sentinel Event"" />")
            StrWer.WriteLine("</series>")
            StrWer.WriteLine("<periods>")
            For i As Integer = 1 To 6
                query_date = DateAdd(DateInterval.Month, -6 + (i), login_date)
                sql = "SELECT a.severe_id , month(c.datetime_report) ,  year(c.datetime_report), COUNT(b.severe_level_id) AS num_severe"
                sql &= " FROM ir_m_severity  a LEFT OUTER JOIN ir_tqm_tab b "
                sql &= " INNER JOIN ir_detail_tab c ON b.ir_id = c.ir_id ON b.severe_level_id = a.severe_id"
                sql &= " WHERE month(c.datetime_report) = " & query_date.Month
                sql &= " AND year(c.datetime_report) = " & query_date.Year
                sql &= " GROUP BY a.severe_id  ,month(c.datetime_report) , year(c.datetime_report)"
                sql &= " ORDER BY a.severe_id "
                ' Response.Write(sql)
                ds = conn.getDataSet(sql, "t1")
                ...
                For s As Integer = 1 To 7
                    If ds.Tables("t1").Rows.Count > 0 Then
                        ...
                        If iRow < ds.Tables("t1").Rows.Count Then
                            'Response.Write(Convert.ToInt32(ds.Tables("t1").Rows(s - 1)("severe_id")))
                            'Response.Write(" irow=" & iRow & ": ")
                            If Convert.ToInt32(ds.Tables("t1").Rows(iRow)("severe_id")) = s Then
    ...
    Sub pineGraph()
        ...
        Dim StrWer As StreamWriter
        Try
            sql = "SELECT COUNT(a.form_id) AS num , c.form_name_en FROM ir_trans_list a INNER JOIN ir_detail_tab b ON a.ir_id = b.ir_id "
            sql &= " INNER JOIN ir_form_master c ON a.form_id = c.form_id"
            ...
            sql &= " GROUP BY a.form_id , c.form_name_en ORDER BY a.form_id "
            '  Response.Write(sql)
            ds = conn.getDataSet(sql, "t1")
            ...
            StrWer.WriteLine("</config>")
            StrWer.WriteLine("<texts>")
            StrWer.WriteLine("<paragraph align=""center"" bold=""true"" color=""0x000066"" size=""16"" x=""45"" y=""10"" width=""500"" text=""Total Number of Incident Report by Category"" />")
            ...
            For i As Integer = 0 To ds.Tables("t1").Rows.Count - 1
                StrWer.WriteLine("<serie " & fColor(i) & "  name=""" & ds.Tables("t1").Rows(i)("form_name_en").ToString & """ value=""" & ds.Tables("t1").Rows(i)("num").ToString & """ />")
    ...
    Sub lineChart()
        ...
        Try
            ...
            StrWer.WriteLine("<config>")
            StrWer.WriteLine("<chartarea x=""0"" y=""30"" width=""500"" height=""220"" backColor1=""0xfafafa"" backColor2=""0xf1f1f1"" xMargin=""15"" yMargin=""15"" borderColor=""0xbdbdbd"" borderThick=""2"" borderAlpha=""1"" />")
            ...
            StrWer.WriteLine("<legends visible=""true"" x=""520"" y=""30"" width=""110"" square=""10"" textSize=""10"" textColor=""0x5e7086"" backColor=""0xFFFFFF"" backAlpha=""1"" borderColor=""0x587b8c"" borderThick=""1"" borderAlpha=""1"" legendBackColor=""0xf2f2f2"" />")
            ...
            StrWer.WriteLine("<texts>")
            StrWer.WriteLine("<paragraph align=""center"" bold=""true"" color=""0x000066"" size=""10"" x=""25"" y=""10"" width=""500"" text=""Total Number of Incident Report By Service Type"" />")
            StrWer.WriteLine("<paragraph align=""center"" bold=""false"" color=""0x000066"" size=""10"" x=""25"" y=""270"" width=""500"" text=""From " & query_date.Day & " " & MonthName(query_date.Month) & " " & query_date.Year & " to " & login_date.Day & " " & MonthName(login_date.Month) & " " & login_date.Year & """  />")
            StrWer.WriteLine("</texts>")
            StrWer.WriteLine("<series>")
            ...
            StrWer.WriteLine("<serie lineColor=""0x000066"" thick=""1"" pointInteriorColor=""0x000099"" pointBorderColor=""0x000066"" tooltipBackColor=""0x0000AA"" tooltipTextColor=""0xFFFFFF"" name=""Other"" />")
            ...
            sql = "SELECT day(date_submit) as day1 , month(date_submit) as month1  , service_type , COUNT(service_type) as num  from ir_detail_tab a"
            sql &= " INNER JOIN ir_trans_list b ON a.ir_id = b.ir_id"
            sql &= " WHERE date_submit BETWEEN '" & query_date.Year & "-" & query_date.Month & "-" & query_date.Day & "' AND "
            sql &= " '" & login_date.Year & "-" & login_date.Month & "-" & login_date.Day & " 23:59:59' "
            sql &= " AND b.is_cancel = 0 AND b.is_delete = 0 AND b.status_id > 0 "
            sql &= " GROUP BY  service_type , day(date_submit), MONTH(date_submit) , YEAR(date_submit)"
            sql &= " ORDER BY YEAR(date_submit) , MONTH(date_submit) ASC , day(date_submit) ASC,  service_type DESC"
            ' Response.Write(sql)
            ' Response.End()
            ds = conn.getDataSet(sql, "t1")
    ...
    Sub bindStackGraph()
        ...
        Try
            ...
            StrWer.WriteLine("<texts>")
            StrWer.WriteLine("<paragraph align=""center"" bold=""true"" color=""0x000066"" size=""16"" x=""45"" y=""10"" width=""500"" text=""Total Number of Incident Report By Customer Segment"" />")
            ...
            sql = "SELECT  month(date_submit) as month1  , customer_segment , COUNT(customer_segment) as num  from ir_detail_tab a"
            sql &= " INNER JOIN ir_trans_list b ON a.ir_id = b.ir_id"
            sql &= " WHERE date_submit BETWEEN '" & query_date.Year & "-" & query_date.Month & "-" & 1 & "' AND "
            sql &= " '" & login_date.Year & "-" & login_date.Month & "-" & login_date.Day & " 23:59:59' "
            sql &= " AND b.is_cancel = 0 AND b.is_delete = 0 AND b.status_id > 0 "
            sql &= " GROUP BY  customer_segment , MONTH(date_submit) , YEAR(date_submit) "
            sql &= " ORDER BY YEAR(date_submit) ASC ,  MONTH(date_submit) ASC ,  customer_segment ASC"
            'Response.Write(sql)
    ...
    Sub bindTotalGrandTopic()
        ...
        Try
            ...
            sql = "SELECT COUNT(d.grand_topic_id) AS num FROM ir_trans_list a INNER JOIN ir_detail_tab b ON a.ir_id = b.ir_id "
    ...
    Sub bindGridIR()
        ...
        Try
            sql = "SELECT d.grand_topic_id , d.grand_topic_name , COUNT(d.grand_topic_id) AS num FROM ir_trans_list a INNER JOIN ir_detail_tab b ON a.ir_id = b.ir_id "
            sql &= " INNER JOIN ir_tqm_tab c ON a.ir_id = c.ir_id"
            sql &= " INNER JOIN ir_topic_grand d ON c.grand_topic = d.grand_topic_id "
            sql &= " WHERE date_submit BETWEEN '" & query_date.Year & "-" & query_date.Month & "-" & 1 & "' AND "
            sql &= " '" & login_date.Year & "-" & login_date.Month & "-" & login_date.Day & " 23:59:59' "
